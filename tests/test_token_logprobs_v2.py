...
PAGE = Path(__file__).resolve().parents[1] / "pages" / "token_probabilities_v2.py"
STANDALONE_APP = Path(__file__).resolve().parents[1] / "token_probabilities_app.py"
...
class TokenLogprobsTests(unittest.TestCase):
    ...
    def test_standalone_entrypoint_loads(self):
        app = AppTest.from_file(str(STANDALONE_APP), default_timeout=20).run()
        self.assertFalse(app.exception)
        self.assertEqual(app.title[0].value, "Probabilidades do próximo token")
